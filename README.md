...
>
> **Open production incident (2026-05-05):** during the first 72h soak, the monitor caught a chain-mutex deadlock on node1 + a downstream mesh-partition fork between node2 and node3. The `BACKUP_LEADER_TIMEOUT 12s → 30s` consensus fix shipped this day prevents one class of fork (the original `h=270` parallel-production race), but the mesh-partition path is still open because node2/node3 only know about each other through node1 as a libp2p relay. Two fixes are pending operator action and documented in [`docs/foundation/22-soak-runbook.md`](docs/foundation/22-soak-runbook.md): (1) add mutual `--bootnode` entries on each systemd unit so node2↔node3 has a direct path; (2) capture a stack trace during the next API hang to pinpoint the deadlocked await. Do not silently wipe the chain to "fix" the soak — the incident log at `~/curs3d-soak/soak.alerts` is the authoritative record.
