...
>
> Live rollout requirement: this storage-format change must be deployed with a coordinated cold restart (`deploy/scripts/full-rollout.sh --wipe`), not a staggered rollout. After any wipe, the Solidity portfolio addresses must be redeployed and `contracts/deployments/1800329576.json` must be refreshed. Do not treat pre-wipe contract addresses as current. Full incident log + gdb stack + recipe in [`docs/foundation/22-soak-runbook.md`](docs/foundation/22-soak-runbook.md).
